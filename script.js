...
(function () {
  ...
  const SYSTEM_REGULATIONS = {
    mechanics: { cameraDistanceZ: 8.5, scrollSensitivity: 0.6, smoothScrollFactor: 0.03, idleBaseRotation: 0.2, particleScrollMultiplier: 2.0, dragSensitivity: 0.005, dragReturnLerp: 0.05 },
    lockSystem: { correctCryptoKey: "ILA-2026", statusChecking: "VERIFYING CRYPTO PROTOCOL... PLEASE WAIT.", statusSuccess: "ACCESS GRANTED. DECRYPTING VOID CORES...", statusFailure: "ACCESS DENIED. INVALID CRYPTOGRAPHIC RECOVERY KEY." },
    loreTexts: {
      anomaly_01: [
        "// BREACH_LOG_0x77A1: CODENAME: LUMINA_CORE",
        "Sistem mendeteksi fragmentasi memori tertanggal 2026-03.",
        "Entitas bernama 'Ila' ditemukan dalam tumpukan array log emosional.",
        ...
        "----------------------------------------------------------------",
        "STATUS: TERISOLASI // JANGAN DIBUKA KEMBALI.",
      ],
      anomaly_02: [
        "// CRITICAL_KERNEL_PANIC: MEMORY_LEAK_DETECTED",
        "Akses root dipaksa oleh USER_ID: THE_MANZZ.",
      ...
      unknownAnomaly: [ "// UNKNOWN_ANOMALY_VECTOR", "Sistem mendeteksi aktivitas ilegal eksternal.", "Mencoba memulihkan sektor..." ],
      secretCyberPanel: [
    ...
    systemStrings: { kernelBooting: "// INITIALIZING MNZ_CORE KERNEL SYSTEM...", kernelSuccess: "// KERNEL SYSTEM INITIALIZATION COMPLETED SUCCESSFUL.", bootFailure: "// BOOT_FAILURE: WebGL initialization context broken.", canvasError: "[VOID_ERROR]: Canvas context null.", scramblePrefix: "Stream_03: ERROR_" },
  };

  const SYSTEM_CONFIG = { canvasId: "main-scene", terminalId: "secret-terminal", textLogId: "secret-text", performanceWarningId: "performance-warning", glitchLayerId: "glitch-layer", lerpFactor: SYSTEM_REGULATIONS.mechanics.smoothScrollFactor, rotationSpeedMultiplier: SYSTEM_REGULATIONS.mechanics.scrollSensitivity, shakeDuration: 1800, typingSpeed: 35 };

  const SYSTEM_STATE = { isCorrupted: false, isShaking: false, currentScrollY: 0, targetScrollY: 0, normalizedScroll: 0, scrollVelocity: 0, fps: 0, lastFrameTime: performance.now(), frameCount: 0, isDragging: false, previousMouseX: 0, previousMouseY: 0, dragRotationX: 0, dragRotationY: 0, targetDragRotationX: 0, targetDragRotationY: 0 };

  class PerformanceMonitor {
    constructor() { this.warningElement = document.getElementById(SYSTEM_CONFIG.performanceWarningId); this.lastUpdate = performance.now(); }
    initialize() {
      if (!this.warningElement) return;
      setTimeout(() => {
        this.warningElement.style.transition = "opacity 1.5s ease";
        this.warningElement.style.opacity = "0";
        setTimeout(() => { this.warningElement.style.display = "none"; }, 1500);
      }, 5000);
    ...
    startFpsTracker() {
      const calculateFps = () => {
        ...
        if (now >= this.lastUpdate + 1000) {
          SYSTEM_STATE.fps = Math.round((SYSTEM_STATE.frameCount * 1000) / (now - this.lastUpdate));
          SYSTEM_STATE.frameCount = 0;
          this.lastUpdate = now;
          const syncIndicator = document.querySelector(".meta-column:nth-child(3) ul li:nth-child(3)");
          if (syncIndicator) syncIndicator.textContent = `SYNC_LEVEL: ${Math.min(100, Math.round(90 + SYSTEM_STATE.fps / 10))}% [${SYSTEM_STATE.fps} FPS]`;
        }
  ...
  class VoidThreeEngine {
    constructor() {
      ...
      this.monolith = null; this.coreMesh = null; this.pointLight = null;
      this.particleSystem = null; this.electrons = []; this.orbits = [];
    }
    ...
    buildAtomGeometry() {
      ...
      const coreMat = new THREE.MeshStandardMaterial({ color: 0x0a0a0f, roughness: 0.2, metalness: 0.9, flatShading: true });
      ...
      const rotations = [ [Math.PI/4, Math.PI/4, 0], [-Math.PI/4, Math.PI/3, 0], [Math.PI/2, Math.PI/6, 0] ];

      for (let i = 0; i < 3; i++) {
        const ringGeo = new THREE.TorusGeometry(1.6, 0.015, 8, 64);
        const ringMat = new THREE.MeshBasicMaterial({ color: orbitColors[i], transparent: true, opacity: 0.35 });
        const orbitRing = new THREE.Mesh(ringGeo, ringMat);
        
        orbitRing.rotation.set(rotations[i][0], rotations[i][1], rotations[i][2]);
        ...
        this.electrons.push({ mesh: electronMesh, orbitIdx: i, angle: (Math.PI * 2 / 3) * i });
      }
    ...
    buildParticleCloud() {
      ...
      const material = new THREE.PointsMaterial({ size: 0.04, color: 0xffffff, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending });
    ...
    initDragControls() {
      const onPointerDown = (x, y, target) => {
        if (target && typeof target.closest === 'function') {
          if (target.closest('button') || target.closest('input') || target.closest('.crypto-lock-box') || target.closest('.action-btn')) {
            return;
          }
        }
      ...
      const onPointerMove = (x, y) => {
        ...
        SYSTEM_STATE.targetDragRotationY += deltaX * SYSTEM_REGULATIONS.mechanics.dragSensitivity;
        SYSTEM_STATE.targetDragRotationX += deltaY * SYSTEM_REGULATIONS.mechanics.dragSensitivity;
        SYSTEM_STATE.previousMouseX = x;
        SYSTEM_STATE.previousMouseY = y;
      };

      const onPointerUp = () => { SYSTEM_STATE.isDragging = false; };
      ...
      window.addEventListener("touchstart", (e) => {
        if (e.touches.length > 0) onPointerDown(e.touches[0].clientX, e.touches[0].clientY, e.target);
      }, { passive: true });
    ...
    update(deltaTime, scrollData) {
      ...
      const idleSpeed = deltaTime * SYSTEM_REGULATIONS.mechanics.idleBaseRotation;
      if (!SYSTEM_STATE.isDragging) {
        SYSTEM_STATE.targetDragRotationX += (0 - SYSTEM_STATE.targetDragRotationX) * SYSTEM_REGULATIONS.mechanics.dragReturnLerp;
        SYSTEM_STATE.targetDragRotationY += (0 - SYSTEM_STATE.targetDragRotationY) * SYSTEM_REGULATIONS.mechanics.dragReturnLerp;
      }
      SYSTEM_STATE.dragRotationX += (SYSTEM_STATE.targetDragRotationX - SYSTEM_STATE.dragRotationX) * 0.1;
      SYSTEM_STATE.dragRotationY += (SYSTEM_STATE.targetDragRotationY - SYSTEM_STATE.dragRotationY) * 0.1;

      this.monolith.rotation.x = -scrollData.velocity * 0.03 - idleSpeed + SYSTEM_STATE.dragRotationX;
      this.monolith.rotation.y = scrollData.normalized * Math.PI * 2 * SYSTEM_CONFIG.rotationSpeedMultiplier + SYSTEM_STATE.dragRotationY;
      ...
      this.electrons.forEach((elec) => {
        ...
        const r = 1.6;
        const vector = new THREE.Vector3(Math.cos(elec.angle) * r, Math.sin(elec.angle) * r, 0);
        vector.applyEuler(this.orbits[elec.orbitIdx].rotation);
        elec.mesh.position.copy(vector);
      });

      this.particleSystem.rotation.x -= scrollData.velocity * 0.02 * SYSTEM_REGULATIONS.mechanics.particleScrollMultiplier + 0.002;
      ...
      if (SYSTEM_STATE.isCorrupted) {
        ...
        const colorLerp = new THREE.Color(0x00ffff).lerp(new THREE.Color(0xff00ff), scrollData.normalized);
  ...
  class ScrollSyncCore {
    constructor() { this.maxScroll = 1; this.prevScrollY = 0; }
    init() {
      this.updateMaxScroll();
      window.addEventListener("scroll", () => { SYSTEM_STATE.targetScrollY = window.scrollY; }, { passive: true });
      window.addEventListener("resize", () => this.updateMaxScroll(), false);
    }
    updateMaxScroll() { this.maxScroll = document.documentElement.scrollHeight - window.innerHeight || 1; }
    calculateMetrics() {
      SYSTEM_STATE.currentScrollY += (SYSTEM_STATE.targetScrollY - SYSTEM_STATE.currentScrollY) * SYSTEM_CONFIG.lerpFactor;
      SYSTEM_STATE.normalizedScroll = Math.max(0, Math.min(1, SYSTEM_STATE.currentScrollY / this.maxScroll));
      SYSTEM_STATE.scrollVelocity = Math.abs(SYSTEM_STATE.currentScrollY - this.prevScrollY);
      this.prevScrollY = SYSTEM_STATE.currentScrollY;
    }
    getScrollData() { return { current: SYSTEM_STATE.currentScrollY, normalized: SYSTEM_STATE.normalizedScroll, velocity: SYSTEM_STATE.scrollVelocity }; }
  }
  ...
  class AnomalyMatrix {
    ...
    init() {
      ...
      if (trigger1) trigger1.addEventListener("click", (e) => { e.preventDefault(); this.triggerAnomalySequence("anomaly_01"); });
      if (trigger2) trigger2.addEventListener("click", (e) => { e.preventDefault(); this.triggerAnomalySequence("anomaly_02"); });
      if (alphaBtn) alphaBtn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); this.flashInterfaceAdvanced(); });
      ...
      if (submitCryptoBtn) submitCryptoBtn.addEventListener("click", (e) => { e.preventDefault(); this.validateCryptoLock(); });
    }
    ...
    executeHeavyBreachSequence() {
      ...
      document.body.classList.add("mega-glitching", "system-shaking", "system-corrupted");
      ...
      setTimeout(() => {
        document.body.classList.remove("mega-glitching", "system-shaking", "system-corrupted");
        if (hackedOverlay) hackedOverlay.classList.remove("triggered");
        if (secretPanel && secretPanelBody) {
          secretPanel.classList.add("active");
          this.typewriterRenderAdvanced(SYSTEM_REGULATIONS.loreTexts.secretCyberPanel, secretPanelBody);
        }
      }, 2000);
    }
    ...
    triggerAnomalySequence(anomalyKey) {
      ...
      if (this.terminal) {
        ...
        const logData = SYSTEM_REGULATIONS.loreTexts[anomalyKey] || SYSTEM_REGULATIONS.loreTexts.unknownAnomaly;
    ...
    executeEarthquakeEffect() {
      ...
      setTimeout(() => { document.body.classList.remove("system-shaking"); SYSTEM_STATE.isShaking = false; }, SYSTEM_CONFIG.shakeDuration);
    }
    ...
    executeGlitchOverlay() {
      ...
      this.playAnomalousBeep();
      setTimeout(() => { document.body.classList.remove("system-corrupted"); SYSTEM_STATE.isCorrupted = false; }, SYSTEM_CONFIG.shakeDuration - 200);
    }
    ...
    flashInterfaceAdvanced() {
      const glitchOverlay = document.getElementById(SYSTEM_CONFIG.glitchLayerId);
    ...
    typewriterRender(lines) {
      ...
      let currentLineIdx = 0; let currentCharIdx = 0;
      ...
      const printNextChar = () => {
        if (currentLineIdx >= lines.length) { this.isTyping = false; return; }
        const currentLineText = lines[currentLineIdx];
        if (currentCharIdx < currentLineText.length) {
          activeContainerLine.textContent += currentLineText.charAt(currentCharIdx);
          currentCharIdx++;
          setTimeout(printNextChar, SYSTEM_CONFIG.typingSpeed);
        } else {
          currentLineIdx++; currentCharIdx = 0;
  ...
  class DataStreamGenerator {
    constructor() {
      this.streamListItems = document.querySelectorAll(".meta-column:nth-child(2) ul li");
      this.chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_//[][]";
    }
    init() { setInterval(() => this.scrambleStreamStrings(), 120); }
    scrambleStreamStrings() {
      ...
      this.streamListItems.forEach((item, index) => {
        if (index === 2 && SYSTEM_STATE.isCorrupted) {
          ...
          item.style.color = "#ff3333"; return;
        }
        if (Math.random() > 0.7) {
          const baseText = item.textContent.split(":")[0];
          let randomHash = "";
          for (let i = 0; i < 6; i++) randomHash += this.chars.charAt(Math.floor(Math.random() * this.chars.length));
